<template>
    <div>
        ...
        <v-card>
            <v-card-title>
                <v-btn class="primary mr-2" to="order/insert">新增</v-btn>
...
<script>
    ...
    data() {
        return {
            ...
            headers: [
                ...
                { text: '訂單編號', value: 'no', sortable: true },
                { text: '成品名稱', value: 'goods_name', sortable: true },
                { text: '最終金額', value: 'final_price', sortable: true },
                { text: '是否結帳', value: 'is_checkout', sortable: true },
    ...
    methods: {
        ...
        updateView(id) {
            var self = this;
            self.$router.push({ name: 'order-update', params: { id: id }  });
        },
        getView(id) {
            var self = this;
            self.$router.push({ name: 'order-get', params: { id: id } });
        },
        deleteData(id){
            var self = this;
        ...
        recoverAmount(id){
            ...
            if (confirm("確定要恢復更改數量嗎 ?")==true){    
                axios.post('/api/order/update/recover', {
                    id:id
                })
                .then(function (response) {
                    if (response.data.result == 'success') {
